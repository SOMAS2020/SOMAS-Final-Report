...
\label{subsec:RulesImplementation}
\subsection{Rules}
\begin{quote}
...
Rules underpin most of the operation of IIGO, and as such we needed their implementation in our project to be very flexible.
Furthermore, it became clear to us that following rules will be difficult for agents unless we provided them the ability to not only check whether they are complicit with them but also the inner workings of the rules so that agents to calculate for themselves how to become complicit.
It became clear to us that we must chose a mathematical representation of rules, since agents would be able to much mroe easily inspect mathematics over a text or functional representation of rules.
The mathematics we settled on was Matrices, and the world of linear algebra that surrounds them.
\linebreak
The premise of this choice was that if a rule could be represented by matrices, then agents could look up every element of the matrix without difficulty. We'd effectively be able to expose the entire operation to the agent without reducing the scope of our rules too much.
To give an example of these matrix based rules, let us consider a simple rule first. 
\linebreak
\linebreak
"If you are expected to pay x amount of tax, the amount of tax you pay must be x"
\linebreak
\linebreak
Quite obviously this is a very simple rule, however it allows us to form a basis for how to represent Rules as matrices.
If our goal is simply to ensure that an agent pays the expected amount of tax, a basic mathematical operation we can perform is subtract the actual tax paid from the expected.
If we get 0, we know this agent has adhered to this rule. 
\[
...
We have now encoded this simply rule as a matrix product. An agent can input their versions of x and y, and perform the calculation.
The agent can then check if their result is 0, if so they are compliant and can pay $y$ tax. Let us revisit our rule, and evolve it a little.
\linebreak
\linebreak
...
\[
    ...
    >= 0  
\]
The addition of the "at least" adds a little bit of complexity to our matrix schema. In that we may not be able to simply check for equality to a constant, and may instead need to check for a certain condition.
...
However if our tax rule looked like this (for whatever reason):
\linebreak
\linebreak
"If you are expected to pay x amount of tax, the amount of tax you pay must be at least x + 5"
\linebreak
\linebreak
Then it would be impossible to capture that using the current matrix calculation, so to capture these cases we introduce a constant to the input list, as below.
\[
    \begin{bmatrix}
        -1 & 1 & -5
    \end{bmatrix}
...
For this, we package with the rule-matrix an "auxiliary" vector containing a list of comparison codes.
\begin{table}[h]
    \centering
    \begin{tabular}{ll}
    \hline
...
\end{table}
\linebreak
The aux code 4 is reserved for when we want the raw result of the matrix multiplication, which is used sparingly for certain rules.
We have now arrived at the representation of rules we are using for our project. For the rule above:
\[
    \textrm{Rule matrix: }
    \begin{bmatrix}
        -1 & 1 & -5
...
\linebreak
\linebreak
"You must pay in tax at least half as many resources as you have and if you've been told to pay tax x you must pay at least tax of x."
\linebreak
\linebreak
In this example there are two separate conditions. We can write both conditions as separate rules.
\linebreak
Condition 1:
\[
    \textrm{Rule matrix: }
...
\[
    ...
    \end{bmatrix}   
\]
However, we can also write this as a single stacked rule:
\[
    \textrm{Rule matrix: }
    \begin{bmatrix}
        0 & 2 & -1 & 0 \\
...
\[
    ...
    \end{bmatrix}   
\]
The real power of this representation of rules is that the mechanics of the calculation is exposed. 
Any agent can inspect the elements of the rule matrix and simply calculate what values they must provide to pass the rule.
...
\[
    ...
    x >= 75
\]
The agent can understand now to pay a minimum of 75. This system allows Agents to both check if they are compliant, and if not they can calculate which values they need to be compliant.
