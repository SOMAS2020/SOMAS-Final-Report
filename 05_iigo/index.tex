...
\begin{enumerate}
    \item Obtaining the required information from agents or other roles. This information includes \emph{allocation requests}, \emph{private pool resource reports} and \emph{rules} proposed by the islands.
    \item Aggregating this information into a format accepted by the interface of presidential functions (usually in form $map[agent] \xrightarrow[]{} information$)
    \item Calling the presidential function only on the currently residing President with aggregated information as inputs
    \item Processing the outcome of the presidential function call
    \item Passing the outcome of the action, or lack thereof, to the archipelago
\end{enumerate}
