...
For the scope of this project we chose only one system of power transfer for the islands to utilise, and it is an election. The islands that hold institutional power are the decision group of the archipelago. They decide on taxes, allocations and sanctions. By holding an election for the institutional roles, the islands are not directly included in the decision group, but they do participate in deciding who will occupy these roles, who makes the aforementioned decisions. An election also opens up another avenue for opinion formation to have an effect. 
\begin{itemize}
...
We note that:
\begin{enumerate}
    \item The Speaker conducts an election for the election of the new Judge.
    \item The Judge conducts an election for the election of the new President.
    \item The President conducts an election for the election of the new Speaker.
\end{enumerate}
Refer to the Figure~ \ref{fig:cycles_in_IIGO} for further clarification about the transfer-of-power cycle.

We introduce a \emph{term} length to further spread out the diversity of the decision group. If the Rule~\ref{rule:roles_must_hold_election} is in play, the roles are obliged to hold an election every N turns. To reduce the scope of the coursework, the term length is defined as a configuration parameter. Thus we reduce the complexity of reasoning the agents have to do with regard to the transfer-of-power of roles.
...
For the simulation we have defined the resources gathered by the islands to be endogenous. Hence we assume that self-organization will consume those resources and institutional-power actions in the IIGO have an associated cost. The institutional-power actions with such a cost are:
...
\item President: Decide the allocation of resources distributed from the common pool.
...
\begin{rule_IIGO} \label{rule:budget}
    ...
    Each role has the \emph{permission} to act on a institutional-power action with a cost if the budget after performing said action would be negative.
 \end{rule_IIGO}
