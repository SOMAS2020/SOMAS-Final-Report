...
The accountability cycle implementation has client and server components, whereby, the decision making functionality is client-side and the monitoring itself occurs server-side.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Input - $roleToBeMonitored$
        \item Output - $decideToMonitor:bool$
    \end{itemize} 
    ...
    \begin{itemize}
        \item Input - $actualMonitoringResult:bool$
        \item Outputs - $decideToAnnounce:bool$, $monitoringResultToAnnounce:bool$
    \end{itemize} 
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \texttt{monitorRole} performs the monitoring of a role as decided by the role accountable. This function calls the client-side monitoring functions and if specified, evaluates the monitoring cache against the rules in play and then broadcasts the monitoring result to all islands.
\end{enumerate}
