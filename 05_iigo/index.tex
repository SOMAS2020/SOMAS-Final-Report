...
\textbf{Non-universal truths.} In our current implementation, there is a global set of rules and a global map which points to the islands in power. Moving these concepts to the individual level could allow for experimentation of what happens when boundaries are not clearly universally defined. In this way, we expect to still maintain that a President has certain unchangeable powers (as stated by "hard-coded" constitutional rules) but the truth about which agent is the President could be blurred. 
